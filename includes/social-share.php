...
function social_media() {
	if (is_single()) {
		...
		echo '
			<div class="post-social-wrap clearfix">';
				if (!empty ($post_socials_text) && ro_get_option('post_socials')) { ?>
				<span class="social-question"><?php echo ro_get_option ('post_socials_text'); ?></span>
				<?php } ?>
				<ul class="post-share-buttons unstyled">
					<li class="twitter">
						<a href="<?php echo esc_url( __('https://twitter.com/share', 'rookie'));?>" class="twitter-share-button" data-url="<?php the_permalink(); ?>" data-text="<?php the_title(); ?>">Tweet</a>
